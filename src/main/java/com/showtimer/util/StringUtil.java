...
public class StringUtil {
	...
	/**
	 * 判断一个list集合是不为空，或者长度不为0
	 * 
	 * @param str
	 * @return
	 */
	public static boolean isNotEmptyList(List str) {
		if (str != null && str.size() > 0 && !str.isEmpty()) {
			return true;
		}
		return false;
	}
	...
	public static boolean containsArray(String str, String[] searchChars) {
		if (str == null || str.length() == 0 || searchChars == null || searchChars.length == 0) {
			return false;
		}
		return Arrays.asList(searchChars).contains(str);
	}
	...
	public static List<String> roundByFindRules(List<AdminRule> adminRules) {
		Map<Integer, String> map = new HashMap<Integer, String>();
		
		for (AdminRule adminRule : adminRules) {
			if (adminRule.getPid() == 0) {
				map.put(adminRule.getId(), adminRule.getName());
			}else {
				map.put(adminRule.getId(), map.get(adminRule.getPid())+"-"+adminRule.getName());
			}
		}
		List<String> list = new ArrayList<String>();
		//遍历map。去除只有两个数值的如 admin-posts，保留 admin-posts-index
	 ...
	 *            字符串
	 * @param defalutValue
	 *            默认值，当str为空或转换失败时，返回该值
	 */
	public static int parseInt(String str, int defalutValue) {
		int i = defalutValue;
		if (isNotEmpty(str)) {
			try {
	 ...
	 * @return
	 */
	public static boolean startWithAndIn(String methedName, String... strings) {

		for (String string : strings) {
			if (methedName.startsWith(string)) {
				return true;
			}
		}
		return false;
